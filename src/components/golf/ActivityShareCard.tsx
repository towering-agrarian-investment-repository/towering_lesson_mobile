...
export const ActivityShareCard = forwardRef<View, ActivityShareCardProps>(function ActivityShareCard({ label, totalReservations, totalDays, totalHours, dailyValues, heatmapStartDate, heatmapEndDate, numDays, memberName, memberImage }, ref) {
        ...
        <View ref={ref} className="w-full overflow-hidden rounded-[28px] border border-border bg-card">
            <View className="bg-primary px-6 pb-5 pt-6">
                <AppText variant="caption" className="font-bold uppercase tracking-[0.16em] text-primary-foreground">
                    My {label === "All time" ? "All" : label} Golf Activity
                </AppText>
                <View className="mt-1 flex-row items-baseline gap-2">
                    <AppText variant="h1" className="text-6xl text-primary-foreground">{totalReservations}</AppText>
                    <AppText variant="h3" className="text-primary-foreground">reservations</AppText>
                </View>
                <View className="mt-3 flex-row gap-3">
                    <ShareMetric value={String(totalDays)} label="Days played" />
                    <ShareMetric value={`${totalHours.toFixed(1)}h`} label="Hours on course" divided />
                </View>
            </View>
            <View className="px-6 pb-3 pt-3">
                <ActivityShareCharts dailyValues={dailyValues} startDate={heatmapStartDate} endDate={heatmapEndDate} numDays={numDays} />
            </View>
